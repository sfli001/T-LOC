...
import time
import datetime
options, args = getopt.getopt(sys.argv[1:], 'o:', ['fastq=', 'bamR=', 'bamT=', 'genome=', 'TDNA=', 'genome_Bwa=', 'TDNA_Bwa=', 'anchor=', 'output=',
                              'resume=', 'read_min_TDNA=', 'read_min_REF=', 'control_bamR=', 'insert=', 'control_fastq=', 'control_ID=', 'Mosaic_length=', 'Sample_name='])
fastq = ""
bamR = ""
control_bamR = ""
control_fastq = ""
...
if (not bamR):
    logging.debug(
        "#######################Run fastq alignment with fastq files#######################\n")
    if (resume == "true" and os.path.exists(
            "%s/Bwa_genome/Done_Bwa_index_genome.txt" % output)):
        genome_Bwa = "%s/Bwa_genome/%s" % (output, re.sub(".*\\/", "", genome))
    if (not genome_Bwa):
        cmd1 = "mkdir %s/Bwa_genome" % output
        cmd2 = "cp %s %s/Bwa_genome/" % (genome, output)
        cmd3 = "bwa index  %s/Bwa_genome/%s" % (
            output, re.sub(".*\\/", "", genome))
        logging.debug(cmd1)
        logging.debug(cmd2)
        logging.debug(cmd3)
        os.system(cmd1)
        ...
        fw.close()
        logging.debug(
            "Output the file for finishing the indexing the genome\n")
    if (resume == "true" and os.path.exists(
            "%s/Reference_%s.sorted.bam.bai" % (REF_path, Sample_name))):
        bamR = "%s/Reference_%s.sorted.bam" % (REF_path, Sample_name)
    if (not bamR):
        cmd_bwa1 = "bwa mem %s %s > %s/Reference_%s.sam 2> %s/Reference_bwa_%s.txt" % (
            genome_Bwa, re.sub(",", " ", fastq), REF_path, Sample_name, REF_path, Sample_name)
        bamR = "%s/Reference_%s.sorted.bam" % (REF_path, Sample_name)
        logging.debug(cmd_bwa1)
        os.system(cmd_bwa1)
        logging.debug(
            "using bwa mem  to do the alignment of reference genome\n")
        cmd1 = "samtools view -bhST %s %s/Reference_%s.sam -o %s/Reference_%s.bam" % (
            genome, REF_path, Sample_name, REF_path, Sample_name)
        cmd2 = "samtools sort -o %s/Reference_%s.sorted.bam %s/Reference_%s.bam" % (
            REF_path, Sample_name, REF_path, Sample_name)
...
if (not bamT):
    ...
    if (not TDNA_Bwa):
        ...
        cmd3 = "bwa index  %s/Bwa_TDNA/%s" % (output,
                                              re.sub(".*\\/", "", TDNA))
        logging.debug(cmd1)
        logging.debug(cmd2)
        logging.debug(cmd3)
    ...
    if (resume == "true" and os.path.exists(
            "%s/TDNA_%s.sorted.bam.bai" % (TDNA_path, Sample_name))):
        bamT = "%s/TDNA_%s.sorted.bam" % (TDNA_path, Sample_name)
    if (not bamT):
        cmd_bwa = "bwa mem %s %s > %s/TDNA_%s.sam 2> %s/TDNA_bwa_%s.txt" % (
            TDNA_Bwa, re.sub(",", " ", fastq), TDNA_path, Sample_name, TDNA_path, Sample_name)
        bamT = "%s/TDNA_%s.sorted.bam" % (TDNA_path, Sample_name)
        logging.debug(cmd_bwa)
        os.system(cmd_bwa)
        logging.debug("using bwa mem files to do the alignment of T-DNA\n")
        cmd1 = "samtools view -hS -F4   %s/TDNA_%s.sam |samtools view -bhST  %s -   -o %s/TDNA_%s.bam" % (
            TDNA_path, Sample_name, TDNA, TDNA_path, Sample_name)
        cmd2 = "samtools sort -o %s %s/TDNA_%s.bam" % (
            bamT, TDNA_path, Sample_name)
...
if (not control_ID):
    logging.debug(
        "#######################Generated the control ID  files or skip the calculation on reference clipped files #######################\n")
if (not control_ID and control_bamR):
    C_bamR = control_bamR.split(",")
    clipped_controlR = ""
    for i in range(0, len(C_bamR)):
        cmd = "samtools view " + C_bamR[i] + "|awk '$6~/[S]/' - > " + \
            Control_path + "/Control_clipped_" + str(i + 1) + ".sam"
        logging.debug(cmd)
        clipped_controlR = clipped_controlR + "," + Control_path + \
            "/Control_clipped_" + str(i + 1) + ".sam"
    clipped_controlR = re.sub(",", "", clipped_controlR)
    control_ID = Control_path + "/Control_id.txt"
    cmd = "Rscript %s/Clipped_control_REF.R %s %s %s" % (
        bin_path, clipped_controlR, control_ID, anchor)
    os.system(cmd)
...
if (not control_ID and control_fastq):
    ...
    for i in range(0, len(C_fastq)):
        cmd_bwa1 = "bwa mem  %s %s > %s/Reference_control_%s.sam 2> %s/Reference_control_%s_bwa.txt" % (
            genome_Bwa, re.sub(",", " ", C_fastq[i]), Control_path, i + 1, Control_path, i + 1)
        cmd = "samtools view " + Control_path + "/Reference_control_" + \
            str(i + 1) + ".sam" + "|awk '$6~/[S]/' - > " + Control_path + "/Control_clipped_" + str(i + 1) + ".sam"
        logging.debug(cmd_bwa1)
        os.system(cmd_bwa1)
        logging.debug(cmd)
        os.system(cmd)
...
if (control_ID):
    cmd = "Rscript %s/Clipped_REF.R %s %s %s %s %s/%s_mosaic_REF_TDNA.txt  %s/%s_mosaic_REF_REF.txt  %s/%s_mosaic_all.txt %s %s %s %s" % (
        bin_path, genome, TDNA, clipped_samR, control_ID, REF_path, Sample_name, REF_path, Sample_name, REF_path, Sample_name, anchor, read_min_TDNA, read_min_REF, insert)
    logging.debug(cmd)
    os.system(cmd)
    logging.debug("output mosaic read from clipped reads in reference\n")

cmd = "Rscript %s/TDNA_Cov.R %s %s %s %s %s %s/%s_TDNA_Cov.pdf " % (
    bin_path, bamR, bamT, genome, TDNA, Sample_name, Result_path, Sample_name)
logging.debug(cmd)
os.system(cmd)
logging.debug("Produce the TDNA coverage\n")

cmd = "Rscript %s/Clipped_TDNA.R %s %s %s  %s/%s_mosaic_REF_TDNA.txt  %s/%s_mosaic_TDNA_TDNA.txt  %s/%s_mosaic_all.txt %s %s %s" % (
    bin_path, genome, TDNA, clipped_samT, TDNA_path, Sample_name, TDNA_path, Sample_name, TDNA_path, Sample_name, anchor, read_min_TDNA, insert)
logging.debug(cmd)
os.system(cmd)
...
if (os.path.exists("%s/%s_mosaic_REF_TDNA.txt" % (REF_path, Sample_name))):
    cmd = "Rscript %s/Mosaic_output.R  %s %s %s/%s_mosaic_REF_TDNA.txt  %s/Mosaic_%s_FromREF.pdf %s/Mosaic_%s_FromREF.fa %s %s" % (
        bin_path, genome, TDNA, REF_path, Sample_name, Result_path, Sample_name, Result_path, Sample_name, Sample_name, Mosaic_length)
    logging.debug(cmd)
    os.system(cmd)
    logging.debug(
        "output Mosaic diagram and fasta files from clipped REF reads\n")
    cmd = "Rscript %s/Coverage_bam.R  %s/%s_mosaic_REF_TDNA.txt  %s %s/Cov_%s_FromREF.pdf %s %s %s" % (
        bin_path, REF_path, Sample_name, bamR, Result_path, Sample_name, REF_path, Sample_name, bin_path)
    logging.debug(cmd)
    os.system(cmd)
    logging.debug(
        "output coverage for mosaic sites get from clipped REF reads\n")

cmd = "Rscript %s/Mosaic_output.R  %s %s %s/%s_mosaic_REF_TDNA.txt  %s/Mosaic_%s_FromTDNA.pdf %s/Mosaic_%s_FromTDNA.fa %s %s" % (
    bin_path, genome, TDNA, TDNA_path, Sample_name, Result_path, Sample_name, Result_path, Sample_name, Sample_name, Mosaic_length)
logging.debug(cmd)
os.system(cmd)
logging.debug(
    "output Mosaic diagram and fasta files from clipped TDNA reads\n")
cmd = "Rscript %s/Coverage_bam.R  %s/%s_mosaic_REF_TDNA.txt  %s %s/Cov_%s_FromTDNA.pdf %s %s %s" % (
    bin_path, TDNA_path, Sample_name, bamR, Result_path, Sample_name, TDNA_path, Sample_name, bin_path)
logging.debug(cmd)
os.system(cmd)
