...
### using reference fasta, TDNA fasta files and TDNA clipped sam files as input
library(ShortRead)
##agrs[1] = reference fasta files
##args[2] = TDNA fasta files
##args[3] = TDNA clipped sam files
##args[4] = out put TDNA_refence mosaic file
##args[5] = out put TDNA_TDNA mosaic file
##args[6] = out put All TDNA  mosaic file
##args[7] = archor length default is 20
##args[8] = minimal required mosaic reads numbers, default is 2
##args[9] = maximal allowed sequence insert length between mosaice reads, default = 30
anchor = 20
if(length(args)>6){
	anchor = as.integer( args[7])
}

lim = 2
if(length(args)>7){
        lim = as.integer( args[8])
}

insert = 30
if(length(args)>8){
        insert = as.integer( args[9])
}

##step1, get rid of read alignment with deletions and no more than two mismatches
...
a1 = data.frame(a1,num = numeric(length(a1[,1])))
for (j in 1:length(a1[,1])){
    a1[j,21] = length(unlist(strsplit(a1[j,13],"[A-Z]")))
}
...
a1 = data.frame(a1[,c(1,2,6,10)],start = a1[,4],end = a1[,4]-1 + as.numeric( gsub("\\d+S|M","",as.character(a1[,6]))))
colnames(a1)[1:4] = c("read_id","FLAG","CIGAR","read_seq")
...
MATCH_TDNA_mosaic = function(genome, TDNA, clip_read){
    ...
    if(clip_read[10]>0){
        pp = sum(vcountPattern(as.character(clip_read[4]), genome,  max.mismatch = 1,with.indels=TRUE))
        if(pp>0){
                clip_read[23] = paste(gsub("false","", clip_read[23]),"perfectPos_Genome",sep=";")
        }
    }
    if(clip_read[11]>0){
        pp = sum(vcountPattern(as.character(reverseComplement(DNAString(as.character(clip_read[4])))), genome, max.mismatch = 1,with.indels=TRUE))
        if(pp>0){
                clip_read[23] = paste(gsub("false","", clip_read[23]),"perfectNeg_Genome",sep=";")
        }
    }
	...
	max_mis = min(5,  as.integer(nchar(s)/10))
        ...
        if(clip_read[14]>0){
                pp = sum(vcountPattern(as.character(clip_read[4]), genome, max.mismatch = max_mis))
                if(pp>0){
                        clip_read[23] = paste(gsub("false","", clip_read[23]),"MismatchPos_Genome",sep="")
                }
        }
        if(clip_read[15]>0){
                pp = sum(vcountPattern(as.character(reverseComplement(DNAString(as.character(clip_read[4])))), genome,  max.mismatch = max_mis))
                if(pp>0){
                        clip_read[23] = paste(gsub("false","", clip_read[23]),"MismatchNeg_Genome",sep="")
                }
        }
        if(clip_read[23]!="false"){
                 return(clip_read)
...
for ( i in 1:length(re_B)){
	...
	re_B[[i]][1,] = MATCH_TDNA_mosaic (genome, TDNA, as.vector(re_B[[i]][1,]))
	re_B[[i]][2:len,10:23] = re_B[[i]][1,10:23]
	if(re_B[[i]][1,23]=="false"){
		re_B[[i]][2,] = MATCH_TDNA_mosaic (genome, TDNA, as.vector(re_B[[i]][2,]))
...
write.table(all, args[6], sep="\t",quote= FALSE, row.names = FALSE,col.names = TRUE)
ALL = all[!grepl("Genome|false",all[,23]),]
ALL_t = table(ALL[,22])
...
##For TDNA_TDNA sequences, there are two types
##Type1, TDNAPos_TDNAPos, type2:TDNANeg_TDNAPos, type3:TDNAPos_TDNAPos,type4, TDNAPos_TDNANeg
ALL_TDNA = data.frame(all[all[,1]==1000,c(1:6,8)],TDNA_seqPOS1 = character(0),TDNA_seqPOS2 = character(0),TDNA_ID1 = character(0),TDNA_ID2 = character(0),match_type = character(0),ID_type= character(0), ID_all=character(0))
for( i in 1:length(ALL[,1])){
        if(ALL[i,7]=="type1"){
            ##seq1 is the part not match in the TNDA and seq2 is the part match in the TDNA
        ...
        ##for type 1, clipped read in the front and map to the position strand of the reference
                if(grepl(";REF_POS", ALL[i,23])){
                  id = gsub("__\\d+__","__", ALL[i,10])
                  re = data.frame(ALL[i,c(1:6,8)],seqREFPos = seq1, seqTDNAPos = seq2,TDNA_ID = ALL[i,22],genome_ID = id ,match_type = "perfect", ID_type= "type1:RefPos_TDNAPos", ID_all= paste(id,":TDNA__",ALL[i,5],sep=""))
                  ALL_REF = rbind(ALL_REF,re)
                }
                if(grepl(";mismatch_REF_POS", ALL[i,23])){
                  id = gsub("__\\d+__","__", ALL[i,14])
                  re = data.frame(ALL[i,c(1:6,8)],seqREFPos = seq1, seqTDNAPos = seq2,TDNA_ID = ALL[i,22],genome_ID = id ,match_type = "mismatch_REF",ID_type= "type1:RefPos_TDNAPos", ID_all= paste(id,":TDNA__",ALL[i,5],sep=""))
                  ALL_REF = rbind(ALL_REF,re)
                }
                if(grepl(";insert_REF_POS", ALL[i,23])){
                  id = gsub("__\\d+__","__", ALL[i,18])
                  re = data.frame(ALL[i,c(1:6,8)],seqREFPos = seq1, seqTDNAPos = seq2,TDNA_ID = ALL[i,22],genome_ID = id ,match_type = "insert_REF",ID_type= "type1:RefPos_TDNAPos", ID_all= paste(id,":TDNA__",ALL[i,5],sep=""))
                  ALL_REF = rbind(ALL_REF,re)
                }
                if(grepl(";REF_NEG", ALL[i,23])){
                  id = gsub("__\\d+$|__\\d+;",";", ALL[i,11])
                  id = gsub(";$","", id)
                  re = data.frame(ALL[i,c(1:6,8)],seqREFPos = seq1RC, seqTDNAPos = seq2,TDNA_ID = ALL[i,22],genome_ID =id,match_type = "perfect",ID_type= "type4:TDNANeg_RefPos", ID_all= paste("TDNA__",ALL[i,5],":",id,sep=""))
                  ALL_REF = rbind(ALL_REF,re)
                }
                ...
                if(grepl(";mismatch_REF_NEG", ALL[i,23])){
                    ...
                    id = gsub(";$","", id)
                  re = data.frame(ALL[i,c(1:6,8)],seqREFPos = seq1RC, seqTDNAPos = seq2,TDNA_ID = ALL[i,22],genome_ID =id,match_type = "mismatch_REF",ID_type= "type4:TDNANeg_RefPos", ID_all= paste("TDNA__",ALL[i,5],":",id,sep=""))
                  ALL_REF = rbind(ALL_REF,re)
                }
                ...
                if(grepl(";insert_REF_NEG", ALL[i,23])){
                    ...
                    id = gsub(";$","", id)
                  re = data.frame(ALL[i,c(1:6,8)],seqREFPos = seq1RC, seqTDNAPos = seq2,TDNA_ID = ALL[i,22],genome_ID =id,match_type = "insert_REF",ID_type= "type4:TDNANeg_RefPos", ID_all= paste("TDNA__",ALL[i,5],":",id,sep=""))
                  ALL_REF = rbind(ALL_REF,re)
                }

                if(grepl(";TDNA_POS", ALL[i,23])){
                    id = gsub("__\\d+__","__", ALL[i,12])
                  re = data.frame(ALL[i,c(1:6,8)],TDNA_seqPOS1 = seq1,TDNA_seqPOS2 = seq2,TDNA_ID1= id,TDNA_ID2 = ALL[i,22] ,match_type = "perfect",ID_type= "type1:TDNAPos_TDNAPos", ID_all= paste(id,":TDNA__",ALL[i,5],sep=""))
                  ALL_TDNA = rbind(ALL_TDNA,re)
                }
                if(grepl(";mismatch_TDNA_POS", ALL[i,23])){
                    id = gsub("__\\d+__","__", ALL[i,16])
                    re = data.frame(ALL[i,c(1:6,8)],TDNA_seqPOS1 = seq1,TDNA_seqPOS2 = seq2,TDNA_ID1= id,TDNA_ID2 = ALL[i,22] ,match_type = "mismatch_TDNA1",ID_type= "type1:TDNAPos_TDNAPos", ID_all= paste(id,":TDNA__",ALL[i,5],sep=""))
                    ALL_TDNA = rbind(ALL_TDNA,re)
                }
                if(grepl(";insert_TDNA_POS", ALL[i,23])){
                    id = gsub("__\\d+__","__", ALL[i,20])
                  re = data.frame(ALL[i,c(1:6,8)],TDNA_seqPOS1 = seq1,TDNA_seqPOS2 = seq2,TDNA_ID1= id,TDNA_ID2 = ALL[i,22] ,match_type = "insert_TDNA1_3end",ID_type= "type1:TDNAPos_TDNAPos", ID_all= paste(id,":TDNA__",ALL[i,5],sep=""))
                  ALL_TDNA = rbind(ALL_TDNA,re)
                }
                ...
                if(grepl(";TDNA_NEG", ALL[i,23])){
                    ...
                    id = gsub(";$","", id)
                  re = data.frame(ALL[i,c(1:6,8)],TDNA_seqPOS1 = seq1RC,TDNA_seqPOS2 = seq2,TDNA_ID1= id,TDNA_ID2 = ALL[i,22] ,match_type = "perfect",ID_type= "type2:TDNANeg_TDNAPos", ID_all= paste(id,":TDNA__",ALL[i,5],sep=""))
                  ALL_TDNA = rbind(ALL_TDNA,re)
                }
                if(grepl(";mismatch_TDNA_NEG", ALL[i,23])){
                    id = gsub("__\\d+$|__\\d+;",";", ALL[i,17])
                    id = gsub(";$","", id)
                  re = data.frame(ALL[i,c(1:6,8)],TDNA_seqPOS1 = seq1RC,TDNA_seqPOS2 = seq2,TDNA_ID1= id,TDNA_ID2 = ALL[i,22],match_type = "mismatch_TDNA1",ID_type= "type2:TDNANeg_TDNAPos", ID_all= paste(id,":TDNA__",ALL[i,5],sep=""))
                  ALL_TDNA = rbind(ALL_TDNA,re)
                }
                if(grepl(";insert_TDNA_NEG", ALL[i,23])){
                    id = gsub("__\\d+$|__\\d+;",";", ALL[i,21])
                    id = gsub(";$","", id)
                  re = data.frame(ALL[i,c(1:6,8)],TDNA_seqPOS1 = seq1RC,TDNA_seqPOS2 = seq2,TDNA_ID1= id,TDNA_ID2 = ALL[i,22],match_type = "insert_TDNA1_3end",ID_type= "type2:TDNANeg_TDNAPos", ID_all= paste(id,":TDNA__",ALL[i,5],sep=""))
                  ALL_TDNA = rbind(ALL_TDNA,re)
                }
        }
	      ...
	      ##seq1 is the part not match in the TNDA and seq2 is the part match in the TDNA
            ...
            seq2RC =as.character(reverseComplement(DNAString(seq2)))
                    if(grepl(";REF_POS", ALL[i,23])){
                        ...
                        id = gsub(";$","", id)
                      re = data.frame(ALL[i,c(1:6,8)],seqREFPos = seq1, seqTDNAPos = seq2,TDNA_ID = ALL[i,22],genome_ID = id,match_type = "perfect",ID_type= "type3:TDNAPos_RefPos", ID_all= paste("TDNA__",ALL[i,6],":",id,sep=""))
                      ALL_REF = rbind(ALL_REF,re)
                    }
                    if(grepl(";mismatch_REF_POS", ALL[i,23])){
                        id = gsub("__\\d+$|__\\d+;",";", ALL[i,14])
                        id = gsub(";$","", id)
                      re = data.frame(ALL[i,c(1:6,8)],seqREFPos = seq1, seqTDNAPos = seq2,TDNA_ID = ALL[i,22],genome_ID = id,match_type = "mismatch_REF",ID_type= "type3:TDNAPos_RefPos", ID_all= paste("TDNA__",ALL[i,6],":",id,sep=""))
                      ALL_REF = rbind(ALL_REF,re)
                    }
                    if(grepl(";insert_REF_POS", ALL[i,23])){
                        id = gsub("__\\d+$|__\\d+;",";", ALL[i,18])
                        id = gsub(";$","", id)
                      re = data.frame(ALL[i,c(1:6,8)],seqREFPos = seq1, seqTDNAPos = seq2,TDNA_ID = ALL[i,22],genome_ID = id,match_type = "insert_REF",ID_type= "type3:TDNAPos_RefPos", ID_all= paste("TDNA__",ALL[i,6],":",id,sep=""))
                      ALL_REF = rbind(ALL_REF,re)
                    }

                     if(grepl(";REF_NEG", ALL[i,23])){
                         id = gsub("__\\d+__","__", ALL[i,11])
                      re = data.frame(ALL[i,c(1:6,8)],seqREFPos = seq1RC, seqTDNAPos = seq2,TDNA_ID = ALL[i,22],genome_ID = id,match_type = "perfect",ID_type= "type2:RefPos_TDNANeg", ID_all= paste(id,":TDNA__",ALL[i,6],sep=""))
                      ALL_REF = rbind(ALL_REF,re)
                    }

                    if(grepl(";mismatch_REF_NEG", ALL[i,23])){
                        id = gsub("__\\d+__","__", ALL[i,15])
                      re = data.frame(ALL[i,c(1:6,8)],seqREFPos = seq1RC, seqTDNAPos = seq2,TDNA_ID = ALL[i,22],genome_ID = id,match_type = "mismatch_REF",ID_type= "type2:RefPos_TDNANeg", ID_all= paste(id,":TDNA__",ALL[i,6],sep=""))
                      ALL_REF = rbind(ALL_REF,re)
                    }

                    if(grepl(";insert_REF_NEG", ALL[i,23])){
                        id = gsub("__\\d+__","__", ALL[i,19])
                      re = data.frame(ALL[i,c(1:6,8)],seqREFPos = seq1RC, seqTDNAPos = seq2,TDNA_ID = ALL[i,22],genome_ID = id,match_type = "insert_REF",ID_type= "type2:RefPos_TDNANeg", ID_all= paste(id,":TDNA__",ALL[i,6],sep=""))
                      ALL_REF = rbind(ALL_REF,re)
                    }
                    ...
                    if(grepl(";TDNA_POS", ALL[i,23])){
                      ...
                      id = gsub(";$","", id)
                      re = data.frame(ALL[i,c(1:6,8)],TDNA_seqPOS1 = seq2,TDNA_seqPOS2 = seq1, TDNA_ID1= ALL[i,22],TDNA_ID2 = id,match_type = "perfect",ID_type= "type3:TDNAPos_TDNAPos", ID_all= paste("TDNA__",ALL[i,6],":",id,sep=""))
                      ALL_TDNA = rbind(ALL_TDNA,re)
                    }
                    if(grepl(";mismatch_TDNA_POS", ALL[i,23])){
                        id = gsub("__\\d+$|__\\d+;",";", ALL[i,16])
                        id = gsub(";$","", id)
                      re = data.frame(ALL[i,c(1:6,8)],TDNA_seqPOS1 = seq2,TDNA_seqPOS2 = seq1, TDNA_ID1= ALL[i,22],TDNA_ID2 = id,match_type = "mismatch_TDNA2",ID_type= "type3:TDNAPos_TDNAPos", ID_all= paste("TDNA__",ALL[i,6],":",id,sep=""))
                      ALL_TDNA = rbind(ALL_TDNA,re)
                    }
                    if(grepl(";insert_TDNA_POS", ALL[i,23])){
                        id = gsub("__\\d+$|__\\d+;",";", ALL[i,20])
                        id = gsub(";$","", id)
                      re = data.frame(ALL[i,c(1:6,8)],TDNA_seqPOS1 = seq2,TDNA_seqPOS2 = seq1, TDNA_ID1= ALL[i,22],TDNA_ID2 = id,match_type = "insert_TDNA2_5end",ID_type= "type3:TDNAPos_TDNAPos", ID_all= paste("TDNA__",ALL[i,6],":",id,sep=""))
                      ALL_TDNA = rbind(ALL_TDNA,re)
                    }

                    if(grepl(";TDNA_NEG", ALL[i,23])){
                        id = gsub("__\\d+__","__", ALL[i,13])
                      re = data.frame(ALL[i,c(1:6,8)],TDNA_seqPOS1 = seq2,TDNA_seqPOS2 = seq1RC, TDNA_ID1= ALL[i,22],TDNA_ID2 = id,match_type = "perfect",ID_type= "type4:TDNAPos_TDNANeg", ID_all= paste("TDNA__",ALL[i,6],":",id,sep=""))
                      ALL_TDNA = rbind(ALL_TDNA,re)
                    }
                    if(grepl(";mismatch_TDNA_NEG", ALL[i,23])){
                        id = gsub("__\\d+__","__", ALL[i,17])
                      re = data.frame(ALL[i,c(1:6,8)],TDNA_seqPOS1 = seq2,TDNA_seqPOS2 = seq1RC, TDNA_ID1= ALL[i,22],TDNA_ID2 = id,match_type = "mismatch_TDNA2",ID_type= "type4:TDNAPos_TDNANeg", ID_all= paste("TDNA__",ALL[i,6],":",id,sep=""))
                      ALL_TDNA = rbind(ALL_TDNA,re)
                    }
                    if(grepl(";insert_TDNA_NEG", ALL[i,23])){
                        id = gsub("__\\d+__","__", ALL[i,21])
                      re = data.frame(ALL[i,c(1:6,8)],TDNA_seqPOS1 = seq2,TDNA_seqPOS2 = seq1RC, TDNA_ID1= ALL[i,22],TDNA_ID2 =  id,match_type = "insert_TDNA2_5end",ID_type= "type4:TDNAPos_TDNANeg", ID_all= paste("TDNA__",ALL[i,6],":",id,sep=""))
                      ALL_TDNA = rbind(ALL_TDNA,re)
                    }
            }
  }
...
write.table(ALL_REF[,-c(2,3,5,6)], args[4], sep="\t",quote= FALSE, row.names = FALSE,col.names = TRUE)
ALL_TDNA = ALL_TDNA[order(ALL_TDNA[,10], ALL_TDNA[,11]),]
ALL_TDNA_t = table(ALL_TDNA[,14])
ALL_TDNA_t = ALL_TDNA_t[ALL_TDNA_t>=lim]
ALL_TDNA = ALL_TDNA[ALL_TDNA[,14] %in% names(ALL_TDNA_t),]
if(dim(ALL_TDNA)[1]>0){
        write.table(ALL_TDNA[,-c(2,3,5,6)], args[5], sep="\t",quote= FALSE, row.names = FALSE,col.names = TRUE)
}
